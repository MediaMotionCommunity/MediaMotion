 ...
 */
	...
	public void ActiveWheelTool(IElement element) {
		...
		this.CursorManagerService.DisabledCursors();
	}

	/// <summary>
	/// Deactivation of the wheel tool.
	/// </summary>
	public void DeactiveWheelTool() {
		this.selectionMade = false;
		this.scalingFactor = 1.0f;
		this.currentState = MenuState.DEACTIVATION;
		this.CursorManagerService.EnabledCursors();
	}
	...
	/// </summary>
	private void Update() {
		if (this.CursorManagerService.GetMainCursor() != null) {
			this.CursorIsPresent();
		} else {
			this.CursorIsAbsent();
		}
	}
	...
	private void CursorIsPresent() {
		Vector3 cursorPosition = this.CursorManagerService.GetMainCursor().transform.position;
		...
		switch (this.currentState) {
			case MenuState.INACTIVE:
				this.MenuIsInactive(parentToFinger, cursorPosition);
				break;
			case MenuState.ACTIVATING:
				this.MenuIsActivating();
				break;
			case MenuState.ACTIVE:
				this.MenuIsActive(cursorPosition);
				break;
			case MenuState.SELECTION:
				this.MenuIsSelection();
				break;
			case MenuState.DEACTIVATION:
				this.MenuIsDeactivation();
				break;
		}

		// Per Button Updates per state
		for (int i = 0; i < this.buttonCount; i++) {
			ArcMaker current = this.RetrieveAndInitArcMaker(i);
			...
			switch (this.currentState) {
				...
				case MenuState.ACTIVATING:
					...
					break;
				case MenuState.ACTIVE:
					this.ButtonMenuIsActive(i, leapScreen, current);
					break;
				case MenuState.SELECTION:
					this.ButtonMenuIsSelection(i, current);
					break;
				case MenuState.DEACTIVATION:
					this.ButtonMenuIsDeactivation(i, current);
					break;
			}
		}
		...
		if (this.currentState == MenuState.ACTIVE) {
			...
			if (this.closest != -1) {
				this.DrawClosestMenu(menuScreen, leapScreen);
			}
		}
	}
	...
	private void CursorIsAbsent() {
		...
		switch (this.currentState) {
			case MenuState.DEACTIVATION:
				this.MenuIsDeactivation();
				for (int i = 0; i < this.buttonCount; i++) {
					this.ButtonMenuIsDeactivation(i, this.RetrieveAndInitArcMaker(i));
				}
				break;
			case MenuState.SELECTION:
				this.MenuIsSelection();
				for (int i = 0; i < this.buttonCount; i++) {
					this.ButtonMenuIsSelection(i, this.RetrieveAndInitArcMaker(i));
				}
				break;
		}
	}
	...
	/// <summary>
	/// Retrieves the and initialize arc maker.
	/// </summary>
	/// <param name="i">The i.</param>
	/// <returns>The current ArcMaker</returns>
	...
	/// <summary>
	/// Menus the is inactive.
	/// </summary>
	/// <param name="parentToFinger">The parent to finger.</param>
	...
	/// <summary>
	/// Buttons the menu is active.
	/// </summary>
	/// <param name="i">The i.</param>
	/// <param name="leapScreen">The leap screen.</param>
	...
	private void ButtonMenuIsSelection(int i, ArcMaker current) {
		if (i != this.closest) {
			...
			current.Bottom = this.SelectionRadius + this.currentSelectionOffset - (this.Thickness / 2.0f);
			current.Top = this.SelectionRadius + this.currentSelectionOffset + (this.Thickness / 2.0f);
		}
	}

	/// <summary>
	/// Buttons the menu is deactivation.
	/// </summary>
	/// <param name="i">The i.</param>
	/// <param name="current">The current.</param>
	...
	/// <summary>
	/// Draws the closest menu.
	/// </summary>
	/// <param name="menuScreen">The menu screen.</param>
	/// <param name="leapScreen">The leap screen.</param>
